...
def test_query_types_functional(app, client):
    ...
    response = client.get("/types", params={"lim": 10, "off": 100})
...
def test_pydantic_types(app, client):
    ...
    @app.get("/types")
    def _get(non_negative: types.NonNegativeInt = 0):
        return {"non_negative": non_negative}

    response = client.get("/types", params={"non_negative": 1})
    assert response.status_code == 200
    assert response.json() == {
        "non_negative": 1,
    }
